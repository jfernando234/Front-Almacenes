...
export class ComprasService {
  ...
  obtenerCompras(): Observable<IComprasList[]> {
    let url = this.apiUrl + `OrdenCompra/ListarAllOrdenesCompra`;
    return this.http.get<any[]>(url).pipe(
      map(data => data.map(item => ({
        idOrdenCompra: item.idOrdenCompra,
        tipoDocumentoId: item.idTipoDocumento,
        ruc: item.numeroDocumento,
        razonSocial: item.nombreProveedor,
        observacion: item.observacion,
        tipoCompraId: item.idTipoCompra,
        total: item.total,
  ...
  obtenerComprasFiltro(inicio: string, fin: string): Observable<IComprasList[]> {
    let url = this.apiUrl + `OrdenCompra/FiltrarOrdenesCompra?inicio=${inicio}&fin=${fin}`;
    return this.http.get<any[]>(url).pipe(
      map(data => data.map(item => ({
        idOrdenCompra: item.idOrdenCompra,
        tipoDocumentoId: item.idTipoDocumento,
